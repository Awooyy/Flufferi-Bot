...
let waifus = [{
        name: "Sano Hikaru",
        images: ["https://i.imgur.com/kusM58R.png"],
    ...
    {
        name: "Hikari Otoya",
        images: ["https://i.imgur.com/rGOr0iy.png"],
...
let secrets = [{
    name: "Meme-Kun",
    images: ["https://i.imgur.com/B0IJLjB.png"],
    personality: "Lewd.",
    gender: "Male",
    mult: 100,
}, ];
...
let coms = [{
        ...
        code: function(message) {
            let out = "**List of usable commands:**\n";
...
client.on("message", async message => {
    ...
    if (command == 'secret') {
        if (args.join(" ").toLowerCase() == "best girl") message.channel.send("Flufferi!");
        if (args[0].toLowerCase() == "itsu.datte") {
            if(secmem.getItem(message.author.id).includes(0)) return;
            secmem.setItem(message.author.id, secmem.getItem(message.author.id) + 0 + ",");
            setAffection(message, 0, true);
            const emb = new Discord.RichEmbed().setColor('#0099ff');
            emb.setTitle("Secret Husbando!");
            emb.setImage(secrets[0].images[0]);
            emb.addField('Name:', secrets[0].name, true);
            emb.addField('Gender:', secrets[0].gender, true);
            emb.addField('Personality:', secrets[0].personality, true);
            message.channel.send(emb);
        }
    }
